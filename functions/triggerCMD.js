        ...
        if (message.member.roles.cache.some(role => `<@&${role.id}>` === configcommand) || configcommand === "e" || message.member.permissions.has(Permissions.FLAGS.ADMINISTRATOR) || whitelist.includes(message.author.id)) {
            ...
            try {
                ...
                if (configlog == "d" || configlog == "1") {
                    ...
                    if (!channellog) {
                        ...
                        setTimeout(function () {
                            ...
                            channellog.send(`__${message.author.tag}__ a utilisé \`${message.content}\` sur **${message.guild.name}** dans *#${message.channel.name}*(${message.channel.id})`);
                        }, 1000)
                    } else {
                        channellog.send(`__${message.author.tag}__ a utilisé \`${message.content}\` sur **${message.guild.name}** dans *#${message.channel.name}*(${message.channel.id})`);
                    }
                ...
                if (!serverlogchannel) {
                    ...
                    serverlogchannel.send(`__${message.author.tag}__ a utilisé \`${message.content}\` sur **${message.guild.name}** dans *#${message.channel.name}*(${message.channel.id})`);
                }
