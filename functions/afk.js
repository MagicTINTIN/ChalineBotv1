                    ...
                    if (message.content.includes(`<@!${user[1].id}>`) || client.users.cache.get(user[1].id).tag) {
                        ...
                        if (listafk[user[1].id]) {
                            ...
                            if (whitelistafklist == "adv") {
                                ...
                                if (whitelistafklist.includes(message.author.id) || message.member.permissions.has(Permissions.FLAGS.MANAGE_MESSAGES)) {
                                    logger.all("mention mais whitelist ou modérateur");
                                } else {
                                    afksettings.shift()
