    ...
    execute(message, args) {
        ...
        if (devlist.includes(message.author.id)) {
            try {
                ...
                if (!args[0] || commandname == "help" || commandname == "?") {
                ...
                if (commandcatg == "n") {
                    ...
                    return message.channel.send(`Erreur : la catégorie doit être n(pour Normal), p(pour Plus) ou h(pour Hard)`);
                }
                existngiffile = false;
                existpgiffile = false;
                existhgiffile = false;
                ...
                setTimeout(function () {
                    if (existngiffile != false || existpgiffile != false || existhgiffile != false) {
                        return message.channel.send(`La commande ,${commandname} existe déjà !`);
                    }
