                    ...
                    if (!fs.existsSync(servgifdir)) {
                        ...
                        fs.writeFile(`${servgifdir}/settings${message.guild.id}.json`, jsonStringcfg, err => {
                            if (err) {
                                ...
                                message.channel.send("Une erreur est survenue, veuillez réessayer, si le problème persiste signalez l'erreur en indiquant ce que vous souhaitiez faire \`;error l'erreur\`\nErreur : \`CMD-NEWGIF-CRWREL-1-MAIN\`")
                            } else {
                                ...
                                setTimeout(function () {
                                    if (!eligibilityread) {
                                        message.channel.send("Nous n'avous pas pu établir votre éligibilité à cette fonctionnalité.\nSi l'erreur persiste merci de report l'erreur \`;error l'erreur\`\nErreur : \`CMD-NEWGIF-CRREEL-1-MAIN\`")
                                    } else {
                                        if (eligibilityread.elgibility == 1) {
                                            if (!args[0]) {
                                                return message.channel.send(`Permet d'ajouter des commandes pour des gifs selon la syntaxe suivante :
...
gifname ne doit contenir que des lettres et pas de caractères spéciaux tels que les accents.`);
                                            ...
                                            if (!args[1]) {
                                                return message.channel.send(`Erreur : Vérifiez le nombre d'arguments !
...
gifname ne doit contenir que des lettres et pas de caractères spéciaux tels que les accents.`);
                                            ...
                                            setTimeout(function () {
                                                if (existngiffile != false || existpgiffile != false || existhgiffile != false) {
                                                    message.channel.send(`La commande ,${commandname} existe déjà nativement dans Chaline !\nPar conséquent la nouvella commande personnalisée sera prioritaire.`);
                                                }
                                                if (existsgiffile != false) {
                                                    return message.channel.send(`La commande ,${commandname} existe déjà sur votre serveur !`);
                                                }
                                                ...
                                                giflisturl = args
                                                giflisturl.shift();
                                                if (giflisturl[giflisturl.length - 1] == "") giflisturl.pop()
    ...
    message.channel.send(giflist[gifchoisi]);
                                                ...
                                                setTimeout(function () {
                                                    ...
                                                    message.channel.send(`Nouvella commande ,${commandname} ajoutée !`);
                                                }, 1000);
                                            }, 1000);
    ...
    message.channel.send(giflist[gifchoisi]);
                                ...
                                fs.writeFile(`${servgifdir}/settings${message.guild.id}.json`, jsonStringcfg, err => {
                                    if (err) {
                                        ...
                                        logger.all(`Successfully wrote gif eligibility of ${guildcfg.id}(${guildcfg.name})`)
                                        client.channels.cache.get(botchannels.gifServerLogChannel).send(`**--- :** ${servgifdir}/settings${message.guild.id}.json du serveur ${message.guild.name} a été créé correctement !`);
                                        message.channel.send("Mise à jour de l'égibilité du serveur en cours, veuillez réessayer, si le problème persiste signalez l'erreur en indiquant ce que vous souhaitiez faire \`;error l'erreur\`\nErreur : \`CMD-NEWGIF-VEWREL-1-MAIN\`")
                                    }
                                })
                            } else {
