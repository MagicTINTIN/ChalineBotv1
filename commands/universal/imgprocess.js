    ...
    previewCard: async function (type, moreinfos) {
        ...
        if (type == "welcomecmd") {
            ...
            try {
                ...
                if (channelidwelcome == "n") { return console.log("nothing exported") } //------------------------------------------------WELCOME ?
            ...
            //-------------------------------------------------INIT
            channelidboost = args[1].replace('<#', '').replace('>', '');
            if (args[2] == "n") { preboostmention = "" } else { preboostmention = args[2].split("+").join(" "); }
            boostmention = args[3];
            if (args[4] == "n") { postboostmention = "" } else { postboostmention = args[4].split("+").join(" "); }
            //-------------------------------------------------GENERAL
            showboostimage = args[5];
            if (args[6] == "n") { backgroundimage = 'https://media.discordapp.net/attachments/820236448590331924/970686295984717824/chalineboost1.jpg' } else { backgroundimage = args[6] }
            if (args[7] == "n") { xcan = 900 } else { if (parseInt(args[7]) <= 1000) { xcan = parseInt(args[7]) } else { xcan = 1000; } }
            if (args[8] == "n") { ycan = 500 } else { if (parseInt(args[8]) <= 1000) { ycan = parseInt(args[8]) } else { ycan = 1000; } }
            //-------------------------------------------------PROFILEPICT
            if (args[9] == "c") { profilepict = "n"; } else { profilepict = args[9]; }
            ...
            try {
                ...
                if (channelidboost == "n") { return console.log("nothing exported") } //------------------------------------------------boost ?
                if (showboostimage == "y") {
                    //--------------------------------------------------CREATE CANVAS
                    const canvas = Canvas.createCanvas(xcan, ycan);
                    const ctx = canvas.getContext('2d');
                    ...
                    const attachment = await new Discord.MessageAttachment(canvas.toBuffer(), 'boostimg.jpg');
                    ...
                    const examplembed = new MessageEmbed()
                        ...
                        .setDescription(`La carte sera envoyée dans ${args[1]}`)
                        .setAuthor({ name: interaction.user.tag, iconURL: interaction.user.displayAvatarURL({ format: 'png' }) })
                        .setImage("attachment://boostimg.jpg")
                    if (boostmention == "m") { examplembed.addFields({ name: "Voici à quoi ressemblera la carte de boost", value: "\u200B" + `${preboostmention}${interaction.member}${postboostmention}` }); }
                    else if (boostmention == "y") { examplembed.addFields({ name: "Voici à quoi ressemblera la carte de boost", value: "\u200B" + `${preboostmention}${postboostmention}` }); }
                    else if (boostmention == "n") { examplembed.addFields({ name: "Voici à quoi ressemblera la carte de boost", value: "\u200B" }); }
            ...
            } catch (err) { console.log('------------------------ERROR PREVIEW BOOSTING-----------------------'); console.error(err); }
        }
    }
}