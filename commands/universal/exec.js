    ...
    command: async function (cmdmsg, embedmessage, cmd) {
        ...
        async function embederror(title, message) {
            ...
            const embed = new MessageEmbed()
                .setTitle('Commande universelle - ' + title)
                .setColor(0xff0000)
        ...
        async function editembed() {
            ...
            if (actStep.graphics) {
                ...
                if (actStep.graphics == "prevcard") {
                    ...
                    await logger.all("à preview : " + messagecollected);
                    //await console.log("à preview : ", messagecollected);
                    const graphembed = await imgprocess.previewCard(command.name + "cmd", messagecollected)

                    //console.log("received")
                    //await console.log("received", graphembed);
            ...
            // Create menus with channel list
                ...
                build.textchannelmenu()
            }


            // Create menus with voice channel list
            else if (actStep.type == "voiceChannelChoice") {
                build.voicechannelmenu()
            }
        ...
        // VOICECHANNELCHOICE type
        async function waitVoiceChannelChoice() {
            await embedmessage.awaitMessageComponent({ filter, componentType: 'SELECT_MENU', time: timedelay })
                .then(interact => {
                    waitcollected = interact.values[0];
                    actionExecutor(actStep.action)
        ...
        async function sendNothingButCancel() {
            const embed = new MessageEmbed()
                .setTitle('Commande universelle - ' + command.name)
        ...
        async function sendConclusion() {
            ...
            if (!serverlogchannel) {
                ...
                serverlogchannel.send(`Résumé de la commande de ${interaction.user.tag} (${interaction.user.id}) dans ${interaction.channel.name}(${interaction.channel.id}) sur ${interaction.guild.name}(${interaction.guild.id}):\n\` ${commandnametosend} ${messagecollected}\``);
            }
        ...
        while (commandrunning && laploop <= 400) {
            ...
            else if (actStep.type == "voiceChannelChoice") await waitVoiceChannelChoice()
